<?php require_once 'partials/header_view.php'; ?>
    ...
    <div class="page-header row no-gutters py-4">
      <div class="col-12 col-sm-4 text-center text-sm-left mb-0">
        <h3 class="page-title">Trình soạn thảo</h3>
      </div>
    </div>
        ...
        <div class="row">
          <div class="col-12">
            <div class="card card-small mb-3">
              <div class="card-body">
                <form id="main_form" method="POST" action="" class="add-new-post" enctype="multipart/form-data">
                  <input type="hidden" name="article_id" value="<?= $context["article"]->id ?>">
                  <input class="form-control form-control-lg mb-3" type="text" placeholder="Tiêu đề" id="title" name="title" value="<?= $context["article"]->title ?>">
                  <div class="form-row">
                    <div class="form-group col-md-6">
                      <textarea class="form-control" placeholder="Tóm tắt" required maxlength="255" id="description" name="description"><?= $context["article"]->description ?></textarea>
                    </div>
                    <div class="form-group col-md-6">
                      <div class="input-group">
                        <div class="input-group-prepend">
                          <span class="input-group-text">Ảnh minh họa mới</span>
                        </div>
                    ...
                    <div class="form-group">
                      <select id="category" class="form-control" name="category_id">
                        ...
                        <?php foreach ($context["categories"] as $category): ?>
                          <option value="<?= $category->id ?>"<?php if ($context["article"]->category->id == $category->id): ?> selected <?php endif ?>><?= $category->name ?></option>
                        <?php endforeach; ?>
                      </select>
                    </div>
                  </li>
                  ...
                  <li class="list-group-item d-flex px-3">
                    <select class="form-control" name="tags[]" id="tags" multiple>
                      <?php foreach ($context["tags"] as $tag): ?>
                          <option value="<?= $tag->id ?>" <?php if (in_array($tag->id,$context["article"]->getTagIDs())): ?> selected <?php endif ?>><?= $tag->name ?></option>
                      <?php endforeach; ?>
                    </select>
                  </li>
                </ul>
      ...
      <div class="col-lg-3 col-md-12">
        ...
        <div class="card card-small card-post card-post--1 mb-3">
          <div id="preview_image" class="card-post__image" style="background-size: contain; background-image: url(<?= STATIC_DIR."/uploads/images/".$context["article"]->image ?>)">
            <span id="preview_category" href="#" class="card-post__category badge badge-pill badge-primary"><?= $context["article"]->category->name ?></span>
          </div>
          <div class="card-body">
            <h5 class="card-title">
    ...
    <script type="text/javascript">
      var quill;
      var content = <?= json_encode($context['article']->content) ?>;
      (function ($) {
          $(document).ready(function () {
            ...
            //$('#editor-container .ql-editor').html(content);
